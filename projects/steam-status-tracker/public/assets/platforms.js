...
function renderDiscord(previous=null) {
  ...
  text('discordJson',JSON.stringify(data,null,2)); renderOverview(); updateDiscordLinkIndicator(); renderDiscordStats(); updateStaleStates(); notifyDiscordChanges(previous,data); updateIntegrationPresentation();
}
...
function renderTelegram(previous=null) {
  ...
  text('telegramJson',JSON.stringify(data,null,2)); renderOverview(); renderTelegramStats(); updateStaleStates(); notifyTelegramChanges(previous,data); updateIntegrationPresentation();
}
