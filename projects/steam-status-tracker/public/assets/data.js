...
async function fetchVisitorCount({ signal } = {}) {
  ...
  const count = raw?.visitors ?? raw?.count;
  return { count: count == null || count === '' ? null : Number(count) };
}
...
class RefreshManager {
  ...
  async refresh(name, { force=false, manual=false } = {}) {
    ...
    resource.promise = (async () => {
      try {
        const raw = await resource.fetcher({ signal:controller.signal });
        if (controller.signal.aborted || resource.controller !== controller) return null;
  ...
  async refreshAll(options={}) {
    ...
    renderSyncTime();
    const failed = results.some(r => r.status === 'rejected') || [...this.resources.values()].some(r => r.error);
    if (options.manual) showToast(failed ? 'Обновление завершилось с ошибками' : 'Все данные обновлены', failed ? 'error' : 'success');
  }
...
function formatDuration(seconds, fallback='Нет данных') {
  if (seconds == null || seconds === '' || !Number.isFinite(Number(seconds)) || Number(seconds)<0) return fallback;
