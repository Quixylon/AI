...
class MotionController {
  constructor() {
    ...
    this.lastFrame = 0;
    this.dirty = true;
    this.selector = '#profileCard, .profile-intro-panel, .social-link, .status-pill, .tracker-topbar, .platform-card, .detail-card, .stats-card, .history-panel, .data-item, .action-button, .ghost-button, .back-link, .tab-link, .filter-button, .copy-button';
  }
  registerAll() {
  ...
  frame(time) {
    ...
    for (const item of this.items) {
      ...
      const max = item.element.id === 'profileCard' ? 1.15 : item.element.matches('.platform-card,.detail-card,.stats-card,.history-panel,.tracker-topbar') ? 1.6 : 2.2;
